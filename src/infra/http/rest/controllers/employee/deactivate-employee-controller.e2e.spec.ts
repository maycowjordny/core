...
describe("Employee update test(e2e)", () => {
    ...
    it("should be able to update a employee ", async () => {
        ...
        const response = await request(app.server).put(`/employee/deactivate/${employee.id}`)
            .set("Authorization", `Bearer ${accessToken}`)
            .send();

        expect(response.statusCode).toEqual(200);
        expect(response.body).toMatchObject({ message: "Employee deactivate successfully!" });
    });
});
